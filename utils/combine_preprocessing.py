...
def save_data(inr, baseline, events, merged_data, base_path, suffix):
    ...
    These are the dataframes that can be used for descriptive analyses, because
    these contain only Warfarin data, and the INR entries are standardized
    across the trials.

    The suffix exists because the data can be preprocesed in a few ways, and
    the suffix allows us to specify/name the dataset we want to save.
    ...
    """
    # Remove misc columns
    for col in ["REMOVE", "START_TRAJ", "END_TRAJ", "SUBJID_NEW_2",
                "MISSING_ID", "REMOVE_PRIOR", "REMOVE_AFTER", "IS_NULL",
                "USUBJID_O_NEW_2", "INR_NEXT_LOWQ", "INR_NEXT_UPQ",
                "INR_BIN_CODES", "INR_BIN", "INR_VALUE_CHANGE", "ACTION",
                "WARFARIN_DOSE_CHANGE_BIN", "FLAG", "WARFARIN_DOSE_CHANGE_SIGN",
                "INR_VALUE_CHANGE_SIGN", "SUBJID_NEW", "FIRST_DAY", "LAST_DAY",
                "START_TRAJ_CUMU", "END_TRAJ_CUMU", "IS_NULL_CUMU", "PREV_DOSE",
                "REMOVE", "INR_MEASURED", "CUMU_MEASUR", "MISSING_ID",
                "START_TRAJ", "END_TRAJ", "SUBJID_NEW_2", "SUBJID_NEW_NEW",
                "USUBJID_O_NEW_NEW"]:
    ...
    feather.write_dataframe(merged_data,
