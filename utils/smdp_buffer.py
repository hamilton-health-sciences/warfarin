...
#         self.event_flag = np.zeros((self.max_size, 1))
    ...
    def prepare_data(self, data, num_actions=7, events_reward=None, discount_factor=0.99):
        ...
        # Done flag
        last_entries = self.data.groupby("USUBJID_O_NEW").apply(pd.DataFrame.last_valid_index).values
        self.data["IS_LAST"] = 0
        ...
        self.data["EVENT_FLAG"] = self.data.groupby("USUBJID_O_NEW")["EVENT_OCCUR"].shift(-1).fillna(0)
