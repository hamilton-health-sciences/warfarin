...
class SMDPReplayBuffer(object):
    ...
    This class has all the code related to creating, storing, loading, and sampling this replay buffer.
    """

    def __init__(self, data_path=None, id_col="USUBJID_O_NEW", state_dim=None, batch_size=None,
                 buffer_size=1e6, device=None):
        """
        The batch size, buffer size, etc are only used when sampling from the replay buffer. They are not needed when creating the buffers.
        """
...
#         self.event_flag = np.zeros((self.max_size, 1))
    ...
    def sample(self, ind=None, return_flag=False):
        ind = np.random.randint(0, self.crt_size, size=self.batch_size) if ind is None else ind
        if return_flag:
            return (
    ...
    def save(self):
        ...
        t1 = time.time()
        print(f"Done saving buffer! Took {t1 - t0:,.2f} seconds. Data stored at: {self.data_path}")
    ...
    def load(self, size=-1, incl_hist=True, seed=42, is_ais=False):
        ...
        if is_ais:
            ...
            self.k, self.state, self.reward, self.action, self.next_state, self.not_done = self.create_buffer(
    ...
    def prepare_data(self, data, num_actions=7, events_reward=None, discount_factor=0.99):
        
        is_training = True if not self.features_ranges else False
        # self.features_ranges = features_ranges
        self.data = data

        print(f"Preparing data for replay buffer...")
        print(f"\t{self.data[self.id_col].nunique():,.0f} patients and {self.data.shape[0]:,.0f} entries")
        t0 = time.time()

        # Remove NaN entries
        state_cols = SMDPReplayBuffer.get_state_features(self.data, incl_flags=True)
        dem_state_cols = [x for x in state_cols if ("INR_VALUE" not in x) and ("WARFARIN_DOSE" not in x)]
        ...
        self.data.loc[:, "REWARD"] = SMDPReplayBuffer.get_reward(self.data, discount_factor=discount_factor,
        ...
        self.data.loc[:, "WARFARIN_DOSE_MULT"] = self.data.groupby(self.id_col)["WARFARIN_DOSE"].shift(-1) / np.maximum(
            self.data["WARFARIN_DOSE"], 0.0001)
        ...
        # Done flag
        last_entries = sample.groupby("USUBJID_O_NEW").apply(pd.DataFrame.last_valid_index).values
        sample["IS_LAST"] = 0
        sample.loc[last_entries, "IS_LAST"] = 1
        sample["DONE_FLAG"] = sample["IS_LAST"].shift(-1)
        sample = sample.drop(columns="IS_LAST")

        # Event flag
        sample["EVENT_OCCUR"] = sample[ADV_EVENTS].sum(axis=1)
        sample["EVENT_FLAG"] = sample.groupby("USUBJID_O_NEW")["EVENT_OCCUR"].shift(-1).fillna(0)
        sample = sample.drop(columns="EVENT_OCCUR")
    ...
    def get_reward(df, discount_factor, events_reward=None):
        ...
        df_exploded_merged['IN_RANGE'] = np.logical_and(df_exploded_merged['INR_VALUE'] >= 2,
        ...
        if events_reward is not None:
            df_exploded_merged['REWARD'] = np.where(df_exploded_merged.loc[:, ADV_EVENTS].sum(axis=1) > 0,
                                                    events_reward,
                                                    df_exploded_merged['REWARD'])

        print(f"\tGetting time elapsed between clinical visits...")
        first_days = df_exploded_merged.groupby(['USUBJID_O_NEW', 'CUMU_INR_MEASURED'])[
            'STUDY_DAY'].first().reset_index().rename(columns={'STUDY_DAY': 'FIRST_STUDY_DAY'})
        ...
        df_exploded_merged['DISC_REWARD'] = (~df_exploded_merged['INR_MEASURED']) * (
                discount_factor ** df_exploded_merged['t'])

        disc_rewards = df_exploded_merged.groupby(['USUBJID_O_NEW', 'CUMU_INR_MEASURED'])[
    ...
    def get_action(df, colname="WARFARIN_DOSE_MULT", num_actions=3, action_space="percent"):
        ...
        if num_actions not in [3, 5, 7]:
            warn(
                f"Could not understand num_actions given: {num_actions}, type: {type(num_actions)}. Will use 7 as default.")
            num_actions = 7
    ...
    @staticmethod
    def get_state(sample, incl_flags=True, incl_hist=True, return_next=False, verbose=True):
    ...
    def create_buffer(self, sample=None, incl_hist=True, shuffle=True, return_flag=False, seed=42):
        if sample is None:
            sample = self.data

        sample_k = sample["k"].values
        sample_state, sample_next_state = (SMDPReplayBuffer.get_state(sample, incl_hist=incl_hist, return_next=True))
