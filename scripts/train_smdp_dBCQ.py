...
#         if os.path.exists(checkpoint_file):
    ...
    while training_iters < args.max_timesteps:
        ...
        np.save(folder_paths["results"] + f"/BCQ_Warfarin_Qloss", loss)

        avg_reward_train, events_rate, reward, actions = eval_policy(policy, valid_replay_buffer, 1,
                                                                     train_replay_buffer)
...
#            device=device, use_rep=use_rep)  # TODO Run weighted importance sampling with learned policy and behavior policy
        ...
        np.save(folder_paths["results"] + f"/BCQ_Warfarin_Reward", evaluations)
...
def eval_policy(policy, valid_replay_buffer, eval_episodes=10, train_replay_buffer=None):
    device = "cuda:0"
    ...
    if train_replay_buffer is not None:
        ...
        train_correct += eval_good_actions(policy, state, num_actions=policy.num_actions)
    ...
    both_actions = pd.DataFrame(
        {"CLINICIAN_ACTION": np.array(action.to("cpu")).transpose()[0], "POLICY_ACTION": pred_action.transpose()[0],
         "ADV_EVENTS": event_flag})
    both_actions.loc[:, "DIFF_ACTIONS"] = both_actions["POLICY_ACTION"] - both_actions["CLINICIAN_ACTION"]
    events_rate = both_actions[both_actions["DIFF_ACTIONS"] == 0]["ADV_EVENTS"].mean()

    print("---------------------------------------")
    print(f"Evaluation over {eval_episodes} episodes: Validation % Reasonable Actions: {avg_reward:.3%}" +
          (f" | Training % Reasonable Actions: {avg_reward_train:.3%}" if train_replay_buffer is not None else ""))
    print("---------------------------------------")
...
def eval_good_actions(policy, state, num_actions):
    pred_action = policy.select_action(np.array(state.to("cpu")), eval=True).transpose()[0]
    inr_state = state[:, 0].cpu().detach().numpy()

    num_good_actions = sum([
        sum(np.where(np.logical_and(inr_state > 0.625, pred_action <= 2), 1, 0)),
...
# # A fixed seed is used for the eval environment
def eval_wis(policy, behav_policy, pol_dataloader, discount, is_demog, device, use_rep=True):
    wis_est = []
    wis_returns = 0
    wis_weighting = 0
    ...
    for representations, obs_state, demog, actions, rewards in pol_dataloader:
        ...
        cur_obs, cur_actions = obs_state[:, :-2, :][:, 1:, :], actions[:, :-2, :][:, 1:, :].argmax(dim=-1)
        ...
        with torch.no_grad():
            ...
            if use_rep:
                ...
                q_val, _, _ = policy.Q(torch.cat((cur_obs.flatten(end_dim=1), cur_demog.flatten(end_dim=1)), dim=-1))
            p_new = F.softmax(q_val, dim=-1).gather(2, cur_actions[:, :,
                                                       None]).squeeze()  # Gather the probabilities from the trained policy
...
#         os.makedirs(args.save_folder)
    ...
    if not os.path.exists(folder_paths["buffers"]):
        sys.exit(f"ERROR: Buffer does not exist for buffer name {args.buffer_name}")
    ...
    wandb.init(id=args.unique_run_id, resume='allow', project=args.project, group=args.buffer_name, name=args.savename)
